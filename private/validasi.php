<?php
	...
	header("Location: ../cetak?status=success");
	// Atasi Undefined
    ...
    if (isset($_POST['submit'])){
        ...
        if ($is_valid) {
			...
			$stmt->execute();
			header("Location: ../status?status=success");
        } elseif (!$is_valid) {
            header("Location: ../ajuan.php?nomor=$nomor&nama=$nama&namaError=$namaError&email=$email&emailError=$emailError&telpon=$telpon&telponError=$telponError&alamat=$alamat&alamatError=$alamatError&pengaduan=$pengaduan&pengaduanError=$pengaduanError&captcha=$captcha&captchaError=$captchaError");
        }
    }
    ...
    function cek_alamat($alamat) {
        ...
        if (!preg_match("/^[a-zA-Z0-9 ]*$/",$alamat)) { // cek fullname bukan huruf
            $alamatError = "Alamat Hanya Boleh Huruf dan Angka (tanpa titik atau koma)";
            $is_valid = false;
